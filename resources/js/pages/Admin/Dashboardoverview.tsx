    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      <header className="sticky top-0 z-50 flex w-full items-center justify-between border-b bg-white px-4 py-3 shadow-sm">
        <div className="flex gap-4 items-center">
          ...
          <Button
            variant="ghost"
            className="text-[#005555] hover:bg-gray-100 ml-2 "
            onClick={() => setShowNotifications(!showNotifications)}
          >
            <FontAwesomeIcon icon={faBell} className="text-2xl" /><span className="text-lg">notification</span>
          </Button>
        </div>
      </header>

      {!isOpen ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 py-8 bg-gray-50">
          {/* Left: Dashboard Stats */}
          <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 h-200 w-190">
          ...
          {/* Notification Panel */}
          {showNotifications && (
            <div className="fixed top-20 right-5 w-80 bg-white px-4 py-6 z-50 shadow-xl rounded-lg">
              ...
              <div className="flex flex-col space-y-3">
                <div className="bg-blue-100 p-3 border-l-4 border-indigo-500 rounded-lg shadow-md hover:shadow-md transition duration-300 transform hover:scale-105">
                ...
                <div className="bg-red-100 p-3 border-l-4 border-red-500 rounded-lg shadow-sm hover:shadow-md transition duration-300 transform hover:scale-105">
                  <p className="text-xs text-gray-500">Alert: Class Timetable Updated</p>
                  <h4 className="text-sm font-semibold text-gray-800">
                    Subject: Math - 9:00 AM Slot Changed
                  </h4>
...
function QuickActionCard({ initial, title, desc, color }: any) {
  return (
    <div className={`flex items-center bg-${color}-100 border-l-4 border-${color}-500 rounded-xl shadow hover:shadow-md transition duration-300 h-31`}>
      <div className={`w-1/3 bg-${color}-200 flex items-center justify-center py-8`}>
        <div className={`text-4xl font-bold text-${color}-600 bg-white w-16 h-14 flex items-center justify-center rounded-full shadow-inner`}>
          {initial}
        </div>
      </div>
      <div className="w-2/3 p-5">
        <p className="text-sm uppercase text-gray-500 tracking-wide mb-1">Quick Action</p>
